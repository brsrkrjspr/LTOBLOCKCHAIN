...
echo "== Peer logs (LTO/HPG/Insurance) last 30 lines =="
for p in peer0.lto.gov.ph peer0.hpg.gov.ph peer0.insurance.gov.ph; do
  echo "--- $p ---"
  docker logs "$p" --tail 30 || true
done

echo "== Chaincode logs (vehicle-registration) last 30 lines =="
docker logs chaincode-vehicle-reg --tail 30 || true
