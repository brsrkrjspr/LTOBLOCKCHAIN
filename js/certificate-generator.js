        ...
        try {
            ...
            try {
                ...
                if (txResponse.success) {
                    if (txResponse.source === 'blockchain' && /^[a-f0-9]{64}$/i.test(txResponse.transactionId || '')) {
            ...
            let qrCodeDataUrl = vehicleData.qr_code_base64 || null;
            if (qrCodeDataUrl) {
                console.log('Using server-generated QR code');
            } else if (blockchainTxId) {
                // If txId was recovered after initial certificate-data load, fetch refreshed QR code
                try {
                    const apiClient = window.apiClient || new APIClient();
                    const refreshedCertData = await apiClient.get(`/api/vehicles/${vehicleData.id}/certificate-data`);
                    if (refreshedCertData.success && refreshedCertData.vehicle?.qr_code_base64) {
                        vehicleData = refreshedCertData.vehicle;
                        qrCodeDataUrl = refreshedCertData.vehicle.qr_code_base64;
                        console.log('Loaded refreshed QR code after transaction ID recovery');
                    }
                } catch (qrRefreshError) {
                    console.warn('Could not refresh QR code after transaction ID recovery:', qrRefreshError);
                }
                if (!qrCodeDataUrl) {
                    console.warn('QR code unavailable even after transaction ID recovery');
                }
            } else {
                console.warn('QR code not available in vehicle data');
            }
