...
if [ -z "$ORDERER_CERT" ] || [ -z "$ORDERER_KEY" ]; then
    echo "❌ Orderer certificates not found"
    exit 1
fi
...
echo "✅ Orderer TLS certificates set up"
...
if [ -z "$PEER_CERT" ] || [ -z "$PEER_KEY" ]; then
    echo "❌ Peer certificates not found"
    exit 1
fi
...
echo "✅ Peer TLS certificates set up"
