...
-- LTO OFFICER Account
-- ============================================
-- Handle existing LTO-OFF-001 employee_id conflict
DO $$
BEGIN
    -- If ltoofficer@lto.gov.ph already exists, update it
    IF EXISTS (SELECT 1 FROM users WHERE email = 'ltoofficer@lto.gov.ph') THEN
        ...
        RAISE NOTICE 'Updated ltoofficer@lto.gov.ph account';
    -- If LTO-OFF-001 exists on a different account, clear it first
    ELSIF EXISTS (SELECT 1 FROM users WHERE employee_id = 'LTO-OFF-001' AND email != 'ltoofficer@lto.gov.ph') THEN
        -- Clear employee_id from the conflicting account
        UPDATE users 
        SET employee_id = NULL 
        WHERE employee_id = 'LTO-OFF-001' AND email != 'ltoofficer@lto.gov.ph';
        
        RAISE NOTICE 'Cleared LTO-OFF-001 from conflicting account';
        
        -- Now create the officer account
        INSERT INTO users (
            email, 
            password_hash, 
            first_name, 
