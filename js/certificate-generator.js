    ...
    async generateCertificate(vehicleData, ownerData) {
        ...
        try {
            ...
            try {
                ...
                } else if (txResponse.success && txResponse.transactionId && txResponse.source === 'vehicle_id') {
                    // Transaction ID is actually a vehicle ID (UUID) - transaction not yet on blockchain
                    console.warn('⚠️ No blockchain transaction ID found - transaction pending');
                    isPending = true;
                    blockchainTxId = null;
                } else {
                    console.warn('⚠️ No transaction ID available - transaction pending');
                    isPending = true;
                    blockchainTxId = null;
                }
            } catch (error) {
                console.warn('Could not fetch transaction ID:', error);
                // Check if vehicle status indicates pending blockchain registration
                if (vehicleData.status === 'PENDING_BLOCKCHAIN' || vehicleData.status === 'SUBMITTED') {
                    isPending = true;
                    blockchainTxId = null;
                } else {
                    // For other statuses, still mark as pending if no tx ID
                    isPending = true;
                    blockchainTxId = null;
                }
            }
