#!/bin/bash
# TrustChain LTO - Run Verification Column Migrations
# Ensures all required columns exist for Insurance, Emission, and HPG workflows

echo "=========================================="
echo "Verification Column Migration Script"
echo "=========================================="
...
echo ""
echo "Step 1: Checking current column status..."
echo "----------------------------------------"

# Check vehicle_verifications auto-verification columns
echo "Checking vehicle_verifications auto-verification columns..."
...
echo ""
echo "Checking transfer_requests approval columns..."
docker exec $DB_CONTAINER psql -U $DB_USER -d $DB_NAME -c "
SELECT 
...
echo ""
echo "Checking clearance_requests.completed_at..."
docker exec $DB_CONTAINER psql -U $DB_USER -d $DB_NAME -c "
SELECT 
...
echo ""
echo "Step 2: Running verification and migration script..."
echo "----------------------------------------"

# Run the verification script
if [ -f "database/verify-verification-columns.sql" ]; then
    docker exec -i $DB_CONTAINER psql -U $DB_USER -d $DB_NAME < database/verify-verification-columns.sql
    echo "✅ Migration script executed"
else
    echo "❌ Migration script not found: database/verify-verification-columns.sql"
    exit 1
fi

echo ""
echo "Step 3: Verifying all columns now exist..."
echo "----------------------------------------"
