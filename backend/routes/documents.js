...
// Search documents - MUST come before /:documentId route to avoid route conflict
router.get('/search', authenticateToken, authorizeRole(['admin']), async (req, res) => {
    try {
        ...
        res.status(500).json({
            ...
            message: process.env.NODE_ENV === 'development' ? error.message : 'Failed to search documents'
        });
    }
});
...
// Search documents - MUST come before /:documentId route
router.get('/search', authenticateToken, authorizeRole(['admin']), async (req, res) => {
    try {
        const { vin, applicationId, vehicleId, documentType, page = 1, limit = 50 } = req.query;
